...
public class WxPortalController {
    ...
    public String authGet(@PathVariable Integer agentId,
        ...
        if (StringUtils.isAnyBlank(signature, timestamp, nonce, echostr)) {
            String bug = "接收到来自微信服务器的认证消息：signature = `" + signature + "`"
                + ", timestamp = `" + timestamp + "`, nonce = `" + nonce
                + "`, echostr = `" + echostr + "`";
            WxCpMessage bugMsg = WxCpMessage.TEXT().content(bug).toUser("xiaohe").build();
            try {
                wxCpService.getMessageService().send(bugMsg);
